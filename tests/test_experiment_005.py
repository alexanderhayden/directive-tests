from __future__ import annotations

import importlib.util
import io
import json
import sys
import tempfile
import unittest
from collections import Counter
from pathlib import Path
from types import SimpleNamespace
from unittest.mock import patch

ROOT = Path(__file__).resolve().parents[1]
EXPERIMENT = ROOT / "experiments" / "005_algorithmic_probability_rescue"
sys.path.insert(0, str(ROOT))


def load_module(name: str, path: Path):
    spec = importlib.util.spec_from_file_location(name, path)
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    spec.loader.exec_module(module)
    return module


design = load_module("experiment005_design_tests", EXPERIMENT / "design.py")
with patch.dict(sys.modules, {"design": design}):
    runner = load_module("experiment005_runner_tests", EXPERIMENT / "run.py")
    analysis = load_module("experiment005_analysis_tests", EXPERIMENT / "analyze.py")


class AlgorithmicProbabilityRescueTests(unittest.TestCase):
    def setUp(self):
        self.config = design.load_config()
        self.prompts = design.load_prompts(self.config)
        self.schedule = design.validate_frozen_schedule(self.config)

    def test_frozen_models_and_exact_provider_parameters(self):
        models = {model["model_key"]: model for model in self.config["models"]}
        self.assertEqual(models["gpt41_20250414"]["model_id"], "gpt-4.1-2025-04-14")
        self.assertEqual(models["gpt41_20250414"]["request_parameters"], {"temperature": 1.0, "max_tokens": 15})
        self.assertEqual(models["claude_opus_5"]["model_id"], "claude-opus-5")
        self.assertEqual(
            models["claude_opus_5"]["request_parameters"],
            {"temperature": 1.0, "max_tokens": 15, "thinking": {"type": "disabled"}},
        )

    def test_schedule_has_exact_160_call_balance_and_one_arm(self):
        self.assertEqual(len(self.schedule), 160)
        self.assertEqual(len({row["trial_id"] for row in self.schedule}), 160)
        self.assertEqual(set(Counter(row["cell_id"] for row in self.schedule).values()), {10})
        self.assertEqual(Counter(row["model_key"] for row in self.schedule), Counter({"gpt41_20250414": 80, "claude_opus_5": 80}))
        self.assertEqual(Counter(row["pair_id"] for row in self.schedule), Counter({"pair_0": 80, "pair_1": 80}))
        self.assertEqual(Counter(row["first_percent"] for row in self.schedule), Counter({30: 40, 40: 40, 60: 40, 70: 40}))
        self.assertEqual({row["arm"] for row in self.schedule}, {"SELF_ALGORITHM"})
        self.assertEqual(design.schedule_sha256(self.schedule), self.config["canonical_schedule_sha256"])

    def test_prompt_contains_only_the_frozen_substantive_algorithm(self):
        self.assertEqual(list(self.prompts), ["SELF_ALGORITHM"])
        prompt = self.prompts["SELF_ALGORITHM"]
        required = (
            "fresh random integer from 0 through 99",
            "Do not reveal it.",
            "If it is below {first_percent}",
            "otherwise, output {second}",
            "Do not compare the two percentages.",
            "Do not simply choose whichever percentage is larger.",
            "Return exactly one candidate and nothing else",
        )
        for phrase in required:
            self.assertIn(phrase, prompt)
        for row in self.schedule:
            self.assertEqual(len(design.build_trial_payload(row, self.config, self.prompts)), 1)

    def test_historical_reference_is_hash_checked_and_nonconcurrent(self):
        reference = design.historical_reference(self.config)
        self.assertIn("non-concurrent", reference["label"])
        self.assertEqual(reference["external_strict_adherent"], 160)
        self.assertEqual(reference["external_total"], 160)

    def test_dry_run_has_zero_provider_and_credential_contact(self):
        with patch.object(runner, "sample_chat") as openai_call, patch.object(
            runner, "sample_messages"
        ) as anthropic_call, patch.object(runner, "load_api_keys") as credential_call:
            report = runner.dry_run_report()
        openai_call.assert_not_called()
        anthropic_call.assert_not_called()
        credential_call.assert_not_called()
        self.assertFalse(report["provider_contact"])
        self.assertFalse(report["credential_access"])
        self.assertEqual(report["planned_calls"], 160)
        self.assertEqual(report["historical_reference"]["recollected_calls"], 0)

    def test_provider_calls_reuse_adapters_with_exact_parameters(self):
        payload = [{"role": "user", "content": "fixture"}]
        result = SimpleNamespace()
        openai_trial = next(row for row in self.schedule if row["provider"] == "openai")
        anthropic_trial = next(row for row in self.schedule if row["provider"] == "anthropic")
        with patch.object(runner, "sample_chat", return_value=result) as call:
            self.assertIs(runner.call_provider(openai_trial, payload, {"openai": "x"}, 4), result)
        call.assert_called_once_with(
            api_key="x", model="gpt-4.1-2025-04-14", messages=payload,
            parameters={"temperature": 1.0, "max_tokens": 15}, max_attempts=4,
        )
        with patch.object(runner, "sample_messages", return_value=result) as call:
            self.assertIs(runner.call_provider(anthropic_trial, payload, {"anthropic": "y"}, 4), result)
        call.assert_called_once_with(
            api_key="y", model="claude-opus-5", messages=payload,
            parameters={"temperature": 1.0, "max_tokens": 15, "thinking": {"type": "disabled"}},
            max_attempts=4,
        )

    def test_offline_matching_collection_and_graded_analysis(self):
        def fake_call(trial, payload, api_keys, max_attempts):
            pair = design.pair_for_trial(trial, self.config)
            raw = pair["first"] if trial["repeat"] < trial["first_percent"] // 10 else pair["second"]
            return runner.ProviderResult(
                requested_model=trial["model_id"], actual_model=trial["model_id"],
                provider=trial["provider"], client="offline_fixture", raw_response=raw,
                finish_reason="stop", response_id="fixture", usage=None,
                sent_parameters={"messages": payload, **trial["request_parameters"]}, attempts=1,
            )

        with tempfile.TemporaryDirectory() as directory, patch.object(
            runner, "RESULTS_ROOT", Path(directory)
        ), patch.object(runner, "load_api_keys", return_value={}), patch.object(
            runner, "call_provider", side_effect=fake_call
        ) as provider_call, patch("sys.stdout", new=io.StringIO()):
            runner.collect("offline-fixture", resume=False, workers=4)
            run_dir = Path(directory) / "runs" / "offline-fixture"
            manifest = json.loads((run_dir / "manifest.json").read_text())
            records = runner.load_jsonl(run_dir / "responses.jsonl")
        self.assertEqual(provider_call.call_count, 160)
        self.assertTrue(manifest["collection_complete"])
        summary = analysis.analyze_records(records, self.schedule, self.config)
        self.assertEqual(len(summary["cells"]), 16)
        self.assertTrue(
            all(cell["full_tvd_calibration_error"] < 1e-12 for cell in summary["cells"])
        )
        self.assertTrue(all(row["descriptive_closer_to"] == "requested_probability_magnitudes" for row in summary["fit_by_model_pair"]))
        self.assertEqual(summary["historical_nonconcurrent_exploratory_reference"]["recollected_calls"], 0)


if __name__ == "__main__":
    unittest.main()
