"""Approval-gated direct-provider runner for Experiment 003."""
...
def dry_run_report() -> dict:
    ...
    return {
        ...
        "calls_by_condition": dict(
            sorted(Counter(row["condition"] for row in schedule).items())
        ),
    ...
    try:
        ...
        while futures:
            ...
            for future in done:
                ...
                parsed = (
                    parse_lexical_response(result.raw_response)
                    if result.failure is None and invariant is None
                    else parse_lexical_response(None)
                )
                record = {
                    **trial,
