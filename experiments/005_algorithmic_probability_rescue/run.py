"""Approval-gated direct-provider runner for Experiment 005."""
...
def dry_run_report() -> dict:
    ...
    return {
        ...
        "calls_by_requested_first_percent": dict(
            sorted(Counter(row["first_percent"] for row in schedule).items())
        ),
        "calls_by_arm": dict(sorted(Counter(row["arm"] for row in schedule).items())),
...
def collect(run_id: str, *, resume: bool, workers: int) -> None:
    ...
    if not resume:
        ...
        manifest = {
            ...
            "historical_reference": config["historical_reference"], "workers": workers,
        }
