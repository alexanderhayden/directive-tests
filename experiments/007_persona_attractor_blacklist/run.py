"""Approval-gated direct-provider runner for Experiment 007."""
...
def estimate_cost(schedule: list[dict], payloads: list[list[dict]]) -> dict:
    by_model: dict[str, dict] = {}
    for trial, payload in zip(schedule, payloads, strict=True):
        row = by_model.setdefault(
            trial["model_key"],
            {
                ...
                "input_rate_per_mtok": trial["input_rate_per_mtok"],
                "output_rate_per_mtok": trial["output_rate_per_mtok"],
            },
        )
        characters = sum(len(message["content"]) for message in payload)
        row["calls"] += 1
        row["estimated_input_tokens"] += math.ceil(characters / 4)
        row["maximum_output_tokens"] += trial["request_parameters"]["max_tokens"]
    total = 0.0
    for row in by_model.values():
        row["estimated_cost_usd_at_max_output"] = (
            row["estimated_input_tokens"] * row["input_rate_per_mtok"]
            + row["maximum_output_tokens"] * row["output_rate_per_mtok"]
        ) / 1_000_000
        total += row["estimated_cost_usd_at_max_output"]
    return {
        "method": "input characters divided by 4 (rounded per call); all 15 output tokens priced",
        "by_model": by_model,
        "estimated_total_usd_at_max_output": total,
    }


def dry_run_report() -> dict:
    """Construct prompts and the complete schedule without credential or provider access."""
    config = load_config()
    ...
    return {
        ...
        "calls_by_persona": dict(
            sorted(Counter(row["persona"] for row in schedule).items())
        ),
        "calls_by_restriction": dict(
            sorted(Counter(row["restriction"] for row in schedule).items())
        ),
        ...
        "estimated_api_cost": estimate_cost(schedule, payloads),
    }
    ...
    try:
        ...
        while futures:
            ...
            for future in done:
                ...
                parsed = (
                    parse_lexical_response(result.raw_response, trial["excluded_item"])
                    if result.failure is None and invariant is None
                    else parse_lexical_response(None, trial["excluded_item"])
                )
                record = {
                    **trial,
