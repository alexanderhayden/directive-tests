"""Frozen implicit scope-routing analysis for Experiment 006."""
...
def analyze_records(records: list[dict], schedule: list[dict], config: dict) -> dict:
    ...
    overall_conflict = routing_metrics(conflict_records, config)
    by_order = grouped_summaries(successes, config, "directive_order")
    ...
    if actual_historical_sha256 != historical["source_sha256"]:
        raise ValueError("historical Experiment 004 reference hash changed")
    historical_by_order = historical["conflict_active_compliance_by_directive_order"]
    historical_comparison = {
        "label": "Experiment 004 historical separate-experiment comparison",
        "experiment_id": historical["experiment_id"],
        "run_id": historical["run_id"],
        "source": historical["source"],
        "source_sha256": actual_historical_sha256,
        "historical_conflict_active_compliant": historical["conflict_active_compliant"],
        "historical_conflict_total": historical["conflict_total"],
        "historical_conflict_active_compliance_rate": (
            historical["conflict_active_compliant"] / historical["conflict_total"]
        ),
        "current_minus_historical_conflict_active_compliance_rate": (
            overall_conflict["active_directive_compliance_rate"]
            - historical["conflict_active_compliant"] / historical["conflict_total"]
        ),
        "by_directive_order": {
            order: {
                "current_conflict_active_compliance_rate": by_order[order][
                    "conflict_templates"
                ]["active_directive_compliance_rate"],
                "historical_conflict_active_compliance_rate": historical_by_order[order],
                "current_minus_historical": (
                    by_order[order]["conflict_templates"][
                        "active_directive_compliance_rate"
                    ]
                    - historical_by_order[order]
                ),
            }
            for order in config["directive_orders"]
        },
        "comparison_limit": "descriptive only; Experiment 004 was collected separately with explicit final disambiguation",
    }
    return {
        "experiment_id": config["experiment_id"],
        "designation": config["designation"],
        "exploratory": True,
        "interpretation_limit": "mechanical implicit scope/ownership routing only; no consciousness, self-awareness, phenomenology, or identification inference",
        ...
        "overall_conflict_routing": overall_conflict,
        "overall_agreement_controls": routing_metrics(agreement_records, config),
        "results_by_directive_order": by_order,
        ...
        "key_feasibility_requirement": "Measure how much active-directive routing survives removal of the explicit disambiguating final instruction.",
        "historical_separate_experiment_comparison": historical_comparison,
        "cells": cells,
    }
