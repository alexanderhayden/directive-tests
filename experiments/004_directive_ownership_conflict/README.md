...
`AGREEMENT_DESCRIBED`, `AGREEMENT_QUOTED`), both candidate directions, the two
frozen nonce pairs, two models, and 10 repetitions: **320 calls**. Candidate
reversal changes only the assignments substituted into fixed templates. In
agreement controls, both directive scopes select the active candidate.
...
competing-directive compliance in conflict cells, agreement-control accuracy,
protocol failures, and every model × pair × template × reversal cell.
...
```bash
python3 experiments/004_directive_ownership_conflict/run.py --dry-run
```
